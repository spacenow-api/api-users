...
class FacebookOAuthStrategy {
  public static MIDDLEWARE = passport.authenticate("facebook-token", {
      ...
      new PassportFacebookToken(
        ...
        async (
          ...
          if (userData) {
            ...
            const userCreated = await authService.registerNewUser(
              userObj,
              "facebook"
            );
            return done(null, {
  ...
  public validate = async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (!req.user || !req.user.id)
        throw new HttpException(400, "Facebook User object is missing.");
      ...
      if (referURL) {
        ...
        if (type === "verification") {
          ...
          const tokenData = Token.create(req.user.id, "guest");
