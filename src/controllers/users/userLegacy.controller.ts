...
class UserLegacyController {
  ...
  private getAllUsersLegacy = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const users = await UserLegacy.findAndCountAll({
        attributes: ['id', 'email', 'emailConfirmed', 'role', 'userBanStatus', 'provider'],
        include: [
          {
            model: UserProfileLegacy,
            as: 'profile'
  ...
  private updateUserProfileLegacy = async (req: Request, res: Response, next: NextFunction) => {
    const { data } = req.body
    try {
      const user = await UserLegacy.findOne({ where: { id: req.query.id } })
      if (!user) {
  ...
  private uploadDocument = async (request: Request, response: Response, next: NextFunction) => {
      ...
      if (!user) next(new HttpException(400, 'User does not exist!'))
        ...
        await uploadDoc.single('file')(request, response, async error => {
          if (error) {
            ...
            try {
              const toSave = Object.assign({}, { userId: id, fileName: file.Location, fileType: file.mimetype })
  ...
  private deleteDocument = async (request: Request, response: Response, next: NextFunction) => {
    const { id } = request.params
  ...
  private getUserDocuments = async (req: Request, res: Response, next: NextFunction) => {
    try {
